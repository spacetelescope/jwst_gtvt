...
def pull_ephemeris():
    """Get JWST's position with respect to the sun and write output locally.
    ...
    -------
    ephemeris_name : str
        Path to text ephemeris used by main in find_tgt_info
    """

    # Construct ephemeris name.
    ephemeris_dir = os.path.join(os.getenv("HOME"), 'jwst_gtvt_ephermerides')
    ephemeris_name = os.path.join(ephemeris_dir, 'horizons_EM_jwst_wrt_sun_2020-2024.txt')

    # Make sure dir exist before writing.
    if not os.path.exists(ephemeris_dir):
        os.makedirs(ephemeris_dir)

    # Make request to ephemeris URL and get results
    # We should probably come up with a better approach for generating the URLs in the future. As for now,
    # this will do...
    ephemeris_url = 'https://ssd.jpl.nasa.gov/horizons_batch.cgi?batch=1&COMMAND=%27-170%27&CENTER=%27500@10%27&MAKE_EPHEM=%27YES%27&TABLE_TYPE=%27VECTORS%27&START_TIME=%272020-01-01%27&STOP_TIME=%272024-12-31%27&STEP_SIZE=%271%20d%27&OUT_UNITS=%27KM-S%27&REF_SYSTEM=%27J2000%27&VECT_CORR=%27LT%27&VEC_LABELS=%27YES%27&VEC_DELTA_T=%27NO%27&CSV_FORMAT=%27YES%27&OBJ_DATA=%27NO%27&VEC_TABLE=%272%27'
    response = urllib.request.urlopen(ephemeris_url)
    lines = response.readlines()

    # Write results to file that will live in home directory
    f = open(ephemeris_name, 'w')
    for line in lines:
        f.write(line.decode("utf-8"))  # Change from binary to utf-8 encoding.
    f.close()
