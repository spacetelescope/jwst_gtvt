...
class Polynomial (object):
    ...
    def apply(self, value):
        ...
        #For each index, raise the input to the power given by that index and multiply by the coefficient
...
class DiscreteBin (HistogramBin):
    ...
    def __str__ (self):
        ...
        #Don't assume a type for the count, as it may not be an integer if normalized.
...
class RangeBin (HistogramBin):
    ...
    def istoo_high (self, value):
        ...
        if (self.upper_inclusive):    #value equal to limit not considered too high
...
class Histogram (object):
    ...
    def __str__ (self):
        ...
        #Don't assume a type for the total, as it may not be an integer if normalized.
    ...
    def normalize(self, total=None):
        ...
        new_histogram = copy.deepcopy(self)   #make a deep copy
...
class ContinuousHistogram (Histogram):
    ...
    def __init__ (self, boundaries, highest_inclusive=False):
        ...
        self.bins.append(RangeBin(min_value=lower_limit))   #Add highest bin
        
        #If highest_inclusive is set, change limit behavior of two highest bins.
    ...
    def store_items (self, value, count=1):
        ...
        #Search all bins up to the next-highest.  If value is not too high for the bin, store the items there.
...
def combine_histograms (histograms):
    ...
    #Initialize the new histogram with properties of the first histogram in the list.    
...
class PoissonDistribution (DiscreteHistogram):
    ...
    def generate_distribution (self):
        ...
        #For each value from 0 up to the next-to-last, compute the Poisson distribution
        #for that value, populate the bin, and keep track of the cumulative total.
        for value in range(len(self.bins) - 1):
            self.bins[value].count = p = self.probability(value)
            cum += p
             
        self.bins[-1].count = 1.0 - cum  #remainder of distribution goes in the last bin
    ...
    def __str__ (self):
        ...
        generic_info = super(self.__class__, self).__str__()  #Call superclass method
    ...
    def retrieve_count_by_value (self, value):
        ...
        #If the value exceeds max_boundary, return None.  Otherwise just use the value as an index
        #into the list of bins.
...
class StatisticalList (list):
    ...
    def compute_statistics (self, min_value = None, max_value=None, max_bins=None):
        ...
        self.sort()     #first sort the list in increasing order -- note this is destructive
        
        #Compute min, max, mean, median, variance, standard deviation, and rms value.
        ...
        #If min_value and/or max_value is specified, limit min/max bin values accordingly.
        ...
        bin_step = (max_bin_value - min_bin_value) / (float(num_bins - 2))  #recalculate bin step

        #Create a histogram with the specified number of evenly spaced boundaries.
        #Number of boundaries is equal to number of bins - 1.
    ...
    def __str__ (self):
        ...
        return('StatisticalList: %d elements, min = %.4f, max = %.4f, mean = %.4f, median = %.4f,\
 variance = %.4f, stdev = %.4f, rms = %.4f'\
...
class Rectangle (object):
    ...
    def motion_tolerant_area (self, motion_length, motion_angle):
        ...
        #Compute the x and y distances to the edge.  A position within the rectangle is only
        #motion-tolerant if it exceeds both of these distances.  The effective length is thus
        #reduced by delta_x and the effective width by delta_y.
...
class Square (Rectangle):
    ...
    def __init__ (self, side):
        ...
        self.side = side
        super(self.__class__, self).__init__(side, side)  #call superclass method with length and width
    ...
    def inner_area(self, excluded_width):
        ...
        #Return the area of a square that is reduced in side length by twice the specified width,
        #because both sides are affected.       
